...
- (void)setIndexes:(NSArray *)indexes
{
    ...
    for (int i = 0; i < indexes.count; i++) {
        if ([[indexes objectAtIndex:i] isKindOfClass:[NSString class]]) {
            NSString *str = [indexes objectAtIndex:i];
            if (!str.length) {
        ...
        alphaLabel.font = [UIFont systemFontOfSize:11];
